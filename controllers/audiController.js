...
const addAudioTask = async (req, res) => {
    ...
    if (!monthId || file === undefined || finalInputIndex === undefined || isNaN(finalInputIndex)) {
    ...
    try {
        ...
        res.status(201).json({ message: "Audio uploaded successfully (Postgres)", filename: file.filename });
    } catch (err) {
        console.error("Error while saving audio (Postgres):", err.message);
        return res.status(500).json({ message: "Error while saving audio", error: err.message });
    }
};
...
const getAudioTasks = async (req, res) => {
    ...
    try {
        ...
        console.error("Error while fetching audio tasks (Postgres):", err.message);
        return res.status(500).json({ message: "Error while fetching audio tasks", error: err.message });
    }
};
...
const deleteAudioTasksByMonth = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: "All audio files were successfully deleted (Postgres)" });
    } catch (err) {
        console.error("Error while deleting audio files (Postgres):", err.message);
        return res.status(500).json({ message: "Error while deleting audio files", error: err.message });
    }
};
