...
const postMockMonth = async (req, res) => {
  ...
  try {
    ...
    console.error('Error adding mock month (Postgres):', err.message);
    return res.status(500).json({ error: 'Error adding mock month', details: err.message });
  }
};
...
const getMockMonths = async (req, res) => {
  try {
    ...
    console.error('Failed to fetch mock months (Postgres):', err.message);
    return res.status(500).json({ error: 'Failed to fetch mock months', details: err.message });
  }
};
...
const getOneMockMonth = async (req, res) => {
  ...
  try {
    ...
    console.error('Server error fetching mock month (Postgres):', err.message);
...
const removeMockMonth = async (req, res) => {
  ...
  try {
    ...
    console.error('Server error deleting mock month (Postgres):', err.message);
    return res.status(500).json({ error: 'Server error', details: err.message });
  }
};
...
const setActivemonth = async (req, res) => {
  ...
  const finalMockId = (mockId === null || mockId === undefined) ? null : Number(mockId);
  ...
  try {
    ...
    if (finalMockId === null) {
      ...
      res.status(200).json({ msg: 'Active mock month set successfully', mockId: finalMockId });
    }
  } catch (err) {
    console.error('Failed to set active mock month (Postgres):', err.message);
    return res.status(500).json({ error: 'Failed to set active mock month', details: err.message });
  }
};
...
const getActivemonth = async (req, res) => {
  try {
    ...
    console.error('Failed to fetch active mock month (Postgres):', err.message);
    return res.status(500).json({ error: 'Failed to fetch active mock month', details: err.message });
  }
};
