...
const startSection = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ message: 'Timer started or was started before (Postgres)' });
  } catch (err) {
    console.error("Timer start xatosi:", err);
    return res.status(500).json({ error: 'Error starting timer', details: err.message });
  }
};
