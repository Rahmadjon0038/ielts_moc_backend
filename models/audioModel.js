...
const createAudioTable = () => {
  ...
  `;
  db.query(query)
    .then(() => console.log("Audio jadval muvaffaqiyatli yaratildi ."))
...
const saveAudio = async (monthId, inputIndex, file) => {
  ...
  const selectQuery =
    "SELECT id, filename FROM audios WHERE month_id = $1 AND input_index = $2";
  ...
  try {
    ...
    const unlinkPromises = audios.map((audio) => {
      ...
      return fs.unlink(filePath).catch((e) => {
        if (e.code !== "ENOENT") {
          console.error(
            `Faylni o'chirishda xatolik (${audio.filename}):`,
            e.message
          );
        }
      });
